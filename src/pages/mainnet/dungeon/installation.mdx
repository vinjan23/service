---
layout: '~/layouts/MarkdownLayout.astro'
title: Installation Node
icon: dungeon
network: Mainnet
chain: dungeon-1
version: v5.0.0
---

### Hardware requirements

The following requirements are recommended for running Defund:

- 4 or more physical CPU cores
- At least 200GB of SSD disk storage
- At least 8GB of memory (RAM)
- At least 100mbps network bandwidth


### Install dependencies Required

```
sudo apt update && sudo apt upgrade -y && sudo apt install curl tar wget clang pkg-config libssl-dev jq build-essential bsdmainutils git make ncdu gcc git jq chrony liblz4-tool -y
```

### Install go

We are gonna use GO Version 1.24.5
If you already have GO installed you can skip this 

```
ver="1.24.5"
cd $HOME
wget "https://golang.org/dl/go$ver.linux-amd64.tar.gz"
sudo rm -rf /usr/local/go
sudo tar -C /usr/local -xzf "go$ver.linux-amd64.tar.gz"
rm "go$ver.linux-amd64.tar.gz"
echo "export PATH=$PATH:/usr/local/go/bin:$HOME/go/bin" >> ~/.bash_profile
source ~/.bash_profile
go version
```

### Install binary

```
cd $HOME
rm -rf dungeonchain 
git clone https://github.com/Crypto-Dungeon/dungeonchain.git
cd dungeonchain
git checkout v5.0.0
make install
```

### Init `Change <MONIKER> with ur Name`

```
dungeond init <node_name> --chain-id dungeon-1
```

### Download Genesis file and addrbook

- Genesis

```
curl -L https://snapshot.vinjan-inc.com/dungeon/genesis.json > $HOME/.dungeonchain/config/genesis.json
```
- Addrbook

```
curl -L https://snapshot.vinjan-inc.com/dungeon/addrbook.json > $HOME/.dungeonchain/config/addrbook.json
```

### Custom Port

```
PORT=165
sed -i -e "s%:26657%:${PORT}57%" $HOME/.dungeonchain/config/client.toml
sed -i -e "s%:26658%:${PORT}58%; s%:26657%:${PORT}57%; s%:6060%:${PORT}60%; s%:26656%:${PORT}56%; s%:26660%:${PORT}61%" $HOME/.dungeonchain/config/config.toml
sed -i -e "s%:1317%:${PORT}17%; s%:9090%:${PORT}90%" $HOME/.dungeonchain/config/app.toml
```

### Set Gas Prices

```
sed -i -e "s|^minimum-gas-prices *=.*|minimum-gas-prices = \"0.05udgn\"|" $HOME/.dungeonchain/config/app.toml
```

### Config pruning

```
sed -i \
-e 's|^pruning *=.*|pruning = "custom"|' \
-e 's|^pruning-keep-recent *=.*|pruning-keep-recent = "100"|' \
-e 's|^pruning-keep-every *=.*|pruning-keep-every = "0"|' \
-e 's|^pruning-interval *=.*|pruning-interval = "20"|' \
$HOME/.dungeonchain/config/app.toml
```
### Indexer Off
```
sed -i 's|^indexer *=.*|indexer = "null"|' $HOME/.dungeonchain/config/config.toml
```

### create service file and start node

```
sudo tee /etc/systemd/system/dungeond.service > /dev/null <<EOF
[Unit]
Description=dungeonchain
After=network-online.target

[Service]
User=$USER
ExecStart=$(which dungeond) start
Restart=on-failure
RestartSec=3
LimitNOFILE=65535

[Install]
WantedBy=multi-user.target
EOF
```

- Enable Service

```
sudo systemctl daemon-reload
sudo systemctl enable dungeond
```

### Statesync
<a href="https://service.vinjan.xyz/mainnet/dungeon/statesync" target="_blank">
  <button style="background-color: green; border: none; color: white; padding: 15px 32px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 10px; box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);" onmouseover="this.style.boxShadow='0 0 0 4px rgba(0,255,0,0.5)'" onmouseout="this.style.boxShadow='0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19)'">Statesync</button>
</a>

### Start
```1
sudo systemctl restart dungeond
sudo journalctl -u dungeond -f -o cat
```

### Create Validator
<a href="https://service.vinjan.xyz/mainnet/dungeon/cheat" target="_blank">
  <button style="background-color: green; border: none; color: white; padding: 15px 32px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 10px; box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);" onmouseover="this.style.boxShadow='0 0 0 4px rgba(0,255,0,0.5)'" onmouseout="this.style.boxShadow='0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19)'">CLI Cheatsheets</button>
</a>
